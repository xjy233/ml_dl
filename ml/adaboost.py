...
def lod_date():
    ...
    for i in range(len(data)):
        if (data.iloc[i, -1] < 12):
            ...
            data.iloc[i, -1] = 0
        # elif( 11>= data.iloc[i, -1] >= 10):
        #     data.iloc[i, -1] = 4
        # elif (13 >= data.iloc[i, -1] >= 12):
        #     data.iloc[i, -1] = 3
        # elif (15 >= data.iloc[i, -1] >= 14):
        #     data.iloc[i, -1] = 2
        # elif (20 >= data.iloc[i, -1] >= 16):
        #     data.iloc[i, -1] = 1
...
def get_Datasets():
    ...
    dataSet,classLabels=make_classification(n_samples=200,n_features=100,n_classes=2)  #make_classification会自动生成一些数据集
...
def createTree(dataSet,alpha="huigui",m=20,max_level=20):   #实现决策树，使用20个特征，深度为10
...
if __name__ == '__main__' :
    ...
    testlables =[];predlables = []
    testlables = dataSet[:,-1].T.tolist()
    RomdomTrees=RondomForest(dataSet,10,alpha="fenlei")   #4棵树，分类。
    print("---------------------RomdomTrees------------------------")
    #print(RomdomTrees[0])
    yhat=predictTree(RomdomTrees,dataSet,alpha="fenlei")
    predlables = yhat.T.tolist()[0]   #当为矩阵时返回为[[1,0,1...]]
    print(yhat.T)
    errorCount = 0
    for i in range(len(dataSet)):
        if predlables[i] != testlables[i]:
            errorCount += 1
    print("错误率:%f%%" % (errorCount / float(len(dataSet)) * 100))
